...
<xsl:stylesheet xmlns:xs="http://www.w3.org/2001/XMLSchema"
   ...
   <xsl:template match="*" priority="1000" mode="M8">
      <svrl:fired-rule xmlns:svrl="http://purl.oclc.org/dsdl/svrl" context="*"/>
		    ...
		    <!--ASSERT -->
      <xsl:choose>
         <xsl:when test="count(@structures:id | @structures:ref | @structures:uri) = (0,1)"/>
         <xsl:otherwise>
            <svrl:failed-assert xmlns:svrl="http://purl.oclc.org/dsdl/svrl"
                                test="count(@structures:id | @structures:ref | @structures:uri) = (0,1)">
               <xsl:attribute name="location">
                  <xsl:apply-templates select="." mode="schematron-select-full-path"/>
               </xsl:attribute>
               <svrl:text>
        An element must have at most one of @structures:id, @structures:ref, or @structures:uri.</svrl:text>
            </svrl:failed-assert>
         </xsl:otherwise>
      </xsl:choose>
   ...
   <xsl:template match="*[@structures:ref]" priority="1000" mode="M9">
      <svrl:fired-rule xmlns:svrl="http://purl.oclc.org/dsdl/svrl" context="*[@structures:ref]"/>
		    ...
		    <!--ASSERT -->
      <xsl:choose>
         <xsl:when test="exists(f-niem:resolve(.))"/>
         <xsl:otherwise>
            <svrl:failed-assert xmlns:svrl="http://purl.oclc.org/dsdl/svrl"
                                test="exists(f-niem:resolve(.))">
               <xsl:attribute name="location">
                  <xsl:apply-templates select="." mode="schematron-select-full-path"/>
               </xsl:attribute>
